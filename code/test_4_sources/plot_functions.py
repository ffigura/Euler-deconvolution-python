...
def plot_classic(data,est_classic,xi,yi,zi,shape):
    ...
    for j in range (2):
        ...
        for i in range (2):
            ...
            plt.title(vet_title[j][i*2], fontsize = 14, loc='center',y=-0.27)
            ...
            ax.set_xlabel('Easting (km)', fontsize = 14)
            ax.tick_params(labelsize=13)
            scat=plt.scatter(est_classic[2*j+i][:,1]/1000., 
                             est_classic[2*j+i][:,0]/1000.,s=40,
                             c=(est_classic[2*j+i][:,2]/1000.),
            ...
            if i == 0 and j == 0:
                ...
                plt.title(vet_title[j][i*2+1], fontsize = 14, loc='center',
                          y=-0.27)
                plt.contourf(yi.reshape(shape)/1000.,xi.reshape(shape)/1000.,
                               data.reshape(shape), 30, cmap='gray')
                ax=plt.gca()
                ...
                scat=plt.scatter(est_classic[2*j+i][:,1]/1000., 
                                 est_classic[2*j+i][:,0]/1000.,s=40,
                                 c=(est_classic[2*j+i][:,3]),cmap=my_cmap,
        ...
        plt.savefig('figures/FIG4_'+str(vet_title2[j])+'.png', 
...
def plot_plateau(data,est_plateau,xi,yi,zi,shape):
    ...
    for j in range (2):
        ...
        for i in range (2):
            ...
            data_plot=np.copy(est_plateau[2*j+i][:,2].reshape(shape))
            data_plot[(est_plateau[2*j+i][:,2].reshape(shape))<minz] = minz
            data_plot[(est_plateau[2*j+i][:,2].reshape(shape))>maxz] = maxz
            ...
            plt.title(vet_title[j][i*2], fontsize = 14, loc='center',y=-0.27)
            im=plt.contourf(yi.reshape(shape)/1000.,xi.reshape(shape)/1000.,
                            est_plateau[2*j+i][:,2].reshape(shape)/1000., 30,
                            cmap='terrain_r',vmin=minz,vmax=maxz)
            ax = plt.gca()
            ax.contour(yi.reshape(shape)/1000.,xi.reshape(shape)/1000.,
                          est_plateau[2*j+i][:,2].reshape(shape)/1000.,15,
            ...
            if i == 0 and j == 0:
                ...
                plt.title(vet_title[j][i*2+1], fontsize = 14, loc='center',
                          y=-0.27)
                im=plt.contourf(yi.reshape(shape)/1000.,xi.reshape(shape)/1000.,
                             est_plateau[2*j+i][:,3].reshape(shape)/1000.,
                             levelsb2,cmap=my_cmap,vmin=minb,vmax=maxb)
                ax=plt.gca()
                ax.contour(yi.reshape(shape)/1000.,xi.reshape(shape)/1000.,
                          est_plateau[2*j+i][:,3].reshape(shape)/1000.,levelsb,
                          colors='k',linestyles='solid')
                ax.set_ylabel('Northing (km)', fontsize = 14)
                ax.set_xlabel('Easting (km)', fontsize = 14)
                ax.tick_params(labelsize=13)
                cbar=plt.colorbar(im,ticks=levelsb,pad=0.01,shrink=1,
                                  format='%d')
                cbar.set_label('$\^b$ (nT)',labelpad=-18,y=-0.03, rotation=0,
                               fontsize=13)
                cbar.ax.tick_params(labelsize=13)
                ...
                plt.title(vet_title[j][i*2+1], fontsize = 14, loc='center',
                          y=-0.27)
                im=plt.contourf(yi.reshape(shape)/1000.,xi.reshape(shape)/1000.,
                             est_plateau[2*j+i][:,3].reshape(shape), levelsb2, 
                             cmap=my_cmap,vmin=minb,vmax=maxb)
                ax=plt.gca()
                ax.contour(yi.reshape(shape)/1000.,xi.reshape(shape)/1000.,
                          est_plateau[2*j+i][:,3].reshape(shape),levelsb,
                          colors='k',linestyles='solid')
                ax.set_ylabel('Northing (km)', fontsize = 14)
                ax.set_xlabel('Easting (km)', fontsize = 14)
                ax.tick_params(labelsize=13)
        ...
        plt.savefig('figures/FIG6_'+str(vet_title2[j])+'.png', 
                    bbox_inches='tight', dpi = 600)
        plt.close('all')
        gc.collect()
