...
def euler_deconv(data,xi,yi,zi,shape,area,SI,windowSize,filt):
    ...
    * estimates_clu : 2d-array
    ...
    moving_window(data,dx,dy,dz,xi,yi,zi,(windowSize,windowSize)):
        ...
        uncertainty = np.sqrt(np.diag(ATA))
        stderror = np.linalg.norm(uncertainty)
        ...
        errorvet[south+windowSize//2][east+windowSize//2]=stderror
    ...
    errorvet=errorvet[delta:-delta,delta:-delta]
    ...
    estimates_plt=np.stack((outx.ravel(),outy.ravel(),outz.ravel(),
                            outb.ravel()),axis=-1)
    #keep only a selected percentage of the solutions ranked by the uncertainty
    estimates=np.stack((estx.ravel(),esty.ravel(),estz.ravel(),estb.ravel(),
                        errorvet.ravel()),axis=-1)
    estimates_clu=np.array(sorted(estimates, key=lambda l:l[-1])) \
                     [:int(len(estimates)*filt),:-1]
    
    return (estimates_plt,estimates_clu)
